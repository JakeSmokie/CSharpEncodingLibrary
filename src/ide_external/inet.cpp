...
static void
skt_close(CRIT_SEC *cs,SOCKET *s)
{
	if (*s == SOCKET_ERROR) return;
	if (cs) CRIT_SEC_ENTER(cs);
	closesocket(*s);
	*s = SOCKET_ERROR;
	if (cs) CRIT_SEC_LEAVE(cs);
}
...
static int
skt_write(transfer_context *tc,char *buf,int n)
{
	int rc,tosend;
	...
	while (n>0)
	{
		...
		tc->last_write_time = time(NULL);
		rc = send(tc->skt,buf,tosend,0);
		CRIT_SEC_ENTER(&tc->cs);
		tc->last_write_time = 0;
		CRIT_SEC_LEAVE(&tc->cs);
	  ...
	  void *cookie,transfer_context *tc)
{
	...
	*pdoc++ = '\0';
	...
	if (!ftp_response(s,220,0,jnk,sizeof(jnk)))			{ skt_close(NULL,&s); return INETERR_BADRESPONSE;	}

	if (!stf(cookie,INETST_AUTHENTICATING,0,NULL,0,0))	{ skt_close(NULL,&s); return INETERR_CANCELED;		}
	if (!skt_printf(s,"USER %s\r\n",user))				{ skt_close(NULL,&s); return INETERR_CANNOTSEND;	}
	if (!ftp_response(s,331,0,jnk,sizeof(jnk)))			{ skt_close(NULL,&s); return INETERR_NOLOGIN;		}

	if (!skt_printf(s,"PASS %s\r\n",pwd))				{ skt_close(NULL,&s); return INETERR_CANNOTSEND;	}
	if (!ftp_response(s,230,0,jnk,sizeof(jnk)))			{ skt_close(NULL,&s); return INETERR_NOLOGIN;		}

	if (!stf(cookie,INETST_STARTING,0,orig_url,0,0))	{ skt_close(NULL,&s); return INETERR_CANCELED;	}
	if (!skt_printf(s,"TYPE I\r\n"))					{ skt_close(NULL,&s); return INETERR_CANNOTSEND;	}
	if (!ftp_response(s,200,0,jnk,sizeof(jnk)))			{ skt_close(NULL,&s); return INETERR_BADRESPONSE;	}

	if (*pdoc)
	{
		if (!skt_printf(s,"CWD %s\r\n",pdoc))			{ skt_close(NULL,&s); return INETERR_CANNOTSEND;	}
		if (!ftp_response(s,250,0,jnk,sizeof(jnk)))		{ skt_close(NULL,&s); return INETERR_BADPATH;		}
	}
	...
	if (!skt_printf(s,"RETR %s\r\n",pfname))			{ skt_close(NULL,&s); return INETERR_CANNOTSEND;	}

	if (!stf(cookie,INETST_CONNECTING,0,ipadr,0,0))		{ skt_close(NULL,&s); return INETERR_CANCELED;		}
	if (skt_open(&tc->skt,ipadr,portnum))				{ skt_close(NULL,&s); return INETERR_NOCONNECT;		}

	if (!ftp_response(s,125,150,jnk,sizeof(jnk)))
	{
		skt_close(NULL,&s);
		skt_close(&tc->cs,&tc->skt);
	...
	if (!ftp_response(s,226,0,jnk,sizeof(jnk)))		{ skt_close(NULL,&s); return INETERR_BADRESPONSE;	}
           ...
           char *referer,void *cookie,transfer_context *tc)
{
	...
	if (user_pwd)
	{
		n = b64_encode((unsigned char *)user_pwd,strlen(user_pwd),
						(unsigned char *)temp_pwd,sizeof(temp_pwd)-1);
		if (n>0)
		{
			temp_pwd[n] = '\0';
	...
	if ( !skt_write(tc,jnk,strlen(jnk)) ||
		!skt_write(tc,posted_data,posted_length) )
	{
		skt_close(&tc->cs,&tc->skt);
		free(posted_data);
          ...
          void *cookie, unsigned timeout_seconds )
{
	...
	transfer_context	tc;
	TID_T				tid;
	char				*p,temp_url[MAX_URL_SIZE],host[MAX_URL_SIZE];
	...
	{
		THREAD_INIT_ATTR();
		if (!THREAD_START(tid,timeout_watch,&tc))
			tc.nsecs = 0;
		else
		{
           ...
           transfer_context *tc)
{
	...
	if (*pdoc=='\0') pdoc = "/";
	if (!stf(cookie,INETST_STARTING,0,pdoc,0,0))	{ skt_close(&tc->cs,&tc->skt); return INETERR_CANCELED;	}
	if (!skt_write(tc,jnk,strlen(jnk)))				{ skt_close(&tc->cs,&tc->skt); return INETERR_CANNOTSEND;	}
           ...
           unsigned timeout_seconds, char *real_url )
{
	...
	transfer_context	tc;
	TID_T				tid;
	char				*p, temp_url[MAX_URL_SIZE],red_url[MAX_URL_SIZE],
			...
			unsigned timeout_seconds)
{
	...
	if ( (p=strchr(host,':')) != NULL ) *p = '\0';
	strcpy(temp_url,url);
	rc = ftp_put(temp_url,transf_f,status_f,cookie,&tc);
