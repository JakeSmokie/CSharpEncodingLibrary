...
static huffnode_t MyWork[256];
static huffnode_t MyTmp[256];
...
//--------------------------------------------------------------------------
bool FindTab(huffnode_t *tmp,int len,unsigned int bits)
{
    if(!tmp)
    {
//        Printf("Huffman init error: no huff node");
        return false;
    }
    if (tmp->zero)
    {
        if(!tmp->one)
        {
//            Printf("Huffman init error: no one in node");
            return false;
        }
        if(len>=32)
        {
//            Printf("Huffman init error: compression screwd");
            return false;
        }
...
{
    if(bit) buf[pos/8]|=Masks[pos%8];
    else    buf[pos/8]&=~Masks[pos%8];
}
//--------------------------------------------------------------------------
int GetBit(unsigned char *buf,int pos)
{
    if(buf[pos/8]&Masks[pos%8]) return 1;
    else                        return 0;
}
//--------------------------------------------------------------------------
bool BuildTree(float *freq)
{
    float min1,min2;
    ...
    for (i=0;i<255;i++)
    {
        ...
        if (minat1<0)
        {
//            Printf("Huffman init error: minatl: %f",minat1);
            return false;
        }
        if (minat2<0)
        {
//            Printf("Huffman init error: minat2: %f",minat2);
            return false;
        }
